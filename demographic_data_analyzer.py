...
def calculate_demographic_data(print_data=True):
    ...
    bachelors = df.loc[df['education'] == 'Bachelors'].value_counts().sum()
    ...
    higher_education = bachelor | masters | doctorate
    lower_education = ~higher_education
    higher_rich = df.loc[higher_education & (df['salary'] == '>50K')].value_counts().sum()
    higher_total = df.loc[higher_education].value_counts().sum()
    lower_rich = df.loc[lower_education &(df['salary'] == '>50K')].value_counts().sum()
    lower_total = df.loc[lower_education].value_counts().sum()

    # percentage with salary >50K
    higher_education_rich = round(higher_rich * 100 / higher_total, 1)
    lower_education_rich = round(lower_rich * 100 / lower_total, 1)
    ...
    rich_country_percentage = rich_country * 100 / population
